...
int    TraceCols[MAX_TRACES] = {HX8357_RED,HX8357_BLUE,HX8357_GREEN};
...
void plot(output_t* Output)
{
  ...
  if(iPnt >= MAX_VALUES) {
    ...
    for(iTr=0; iTr<MAX_TRACES; iTr++) {
      ...
      tft.setTextColor(TraceCols[iTr]);  tft.setTextSize(FONT_HEIGHT);
  ...
  if((((iPnt-1) % PLOT_UPDATE) == 0) || (iPnt == 0)) {
    ...
    tft.setTextColor(HX8357_BLACK);  tft.setTextSize(FONT_HEIGHT);
    tft.println(timeStr);
    sprintf(timeStr, "M%d %.1fs\n", Output->NeuronBehaviour, Output->currentMicros /1E6);
    tft.setCursor(dxInfo/2, dyPlot);
    tft.setTextColor(HX8357_WHITE);  tft.setTextSize(FONT_HEIGHT);
    tft.println(timeStr);
  }
}
