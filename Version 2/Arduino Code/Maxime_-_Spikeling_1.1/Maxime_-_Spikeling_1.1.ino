/* //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                                      ...
                                      Spikeling v2.1. By T Baden, Sussex Neuroscience, UK (www.badenlab.org)                                       

2019.01.19 - Designed by T Baden, adapted by T Euler & MJY Zimmermann                              //  
...
int   nosound         = 0;    // default = 0, click on spike + digi out port active. 1 switches both off
int   noled           = 0;    // default = 1, 1 switches the LED off
int   FastMode        = 0;    // default = 0; 
                              // if >0, the script is more optimised for speed by removing some of the serial outputs at the
...
int   StimMode        = 1;    // default = 1
                              // StimMode = 1: Stimulator generate pulse frequency being controlled by Syn1Dial
                              // Syn1Mode = 2: Stimulator generate random Noise sequences (for reverse correlation)
                              // Note: this is being read into the StimMode Array below. This can also be manually set for each Mode, if desired by
                              // simply replacing the StimMode entries in this array with 1 or 2
...
int   Array_StimMode[] =  {StimMode,StimMode,StimMode,StimMode,StimMode}; // Stimulus mode - 1: Steps, 2: 50 Hz binary noise out (for reverse correlation)
...
int DigiOutStep = 0;        // stimstep counter for stimulator mode
...
void loop(void) {
  ...
  if (buttonState == HIGH) {
    ...
    for (int modeblink = 0; modeblink < NeuronBehaviour +1; modeblink++) {
      digitalWriteHelper(ModeLED, HIGH); // Blinks the onboard LED according to which programme is selected
      delay(150);
      digitalWriteHelper(ModeLED, LOW);
      delay(150);
    } 
  }

 // NOTE: the below analogRead functions take some microseconds to execute so to speed up the model they could be called less frequently or removed entirely.
 // To remove them, simply change e.g. PotVal = analogRead(VmPotPin); to PotVal = 512;
  ...
  AnalogInPotVal = analogReadHelper(AnalogPotPin); // 0:1023, Vm - reads same pot as Synapse 2!
  ...
  StimPotVal = analogReadHelper(StimPotPin); // 0:1023, Vm
  ...
  if (Array_StimMode[NeuronBehaviour]==1){ // if in Pulse Mode
    if (DigiOutStep<Stimulator_Val){
       digitalWriteHelper(StimulatorPin, HIGH); 
       Stim_State = 1;
    } else {
       digitalWriteHelper(StimulatorPin, LOW);
       Stim_State = 0;
    }
    DigiOutStep+=1;
    if (DigiOutStep>=Stimulator_Val*2) {
      DigiOutStep = 0;
      Stimulator_Val = round((StimPotVal/1023) * 100); // also calculate 0-100 range variable 
  ...
  if (Array_StimMode[NeuronBehaviour]==2){ // if in Noise Mode
